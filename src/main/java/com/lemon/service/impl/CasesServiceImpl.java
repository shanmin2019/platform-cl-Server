...
/*
            caseParamValue.setApiRequestParamValue(requestParam.getExampleData());
*/
     ...
     */
    @Override
    public List<CaseListVO> showCaseUnderSuiteId(Integer suiteId) {
    ...
    public Result updateCases(CaseEditVO caseEditVO) {
        try {
            ...
            //2、再更新casaeparamvalue表  --有新增删除更新等操作，--->拿到  已有数据
            List<CaseParamValue> list = caseParamValueService.list(queryWrapper);
            if(list.size()>0){
                for (CaseParamValue caseParamValue : list){
                    caseParamValueService.removeById(caseParamValue.getId());
                    System.out.println("删除removeById"+caseParamValue.getId()+"成功~");
                }

            }
            List<ApiRequestParam> requestParams = caseEditVO.getRequestParams();
            if(requestParams.size()>0){
                List<CaseParamValue> list1 = new ArrayList<>();
                for(ApiRequestParam apiRequestParam : requestParams){
                    ...
                    caseParamValue.setCreateUser(user.getId());
                    list1.add(caseParamValue);
                }
                caseParamValueService.saveOrUpdateBatch(list1);
            }
...
//			updateCaseParam.add(caseParamValue);
    ...
    private void updateCaseParamValueTable(CaseEditVO caseEditVO, User user, QueryWrapper queryWrapper) {
        ...
        //a、删除多余数据
        if (caseParamAll.size() > 0) {
            for (CaseParamValue caseParamValue : caseParamAll) {
                caseParamValueService.removeById(caseParamValue.getId());
            }
        }
        //b、更新或者新增
        if (apiRequestParams.size() > 0) {
            List<CaseParamValue> updateCaseParamList = new ArrayList<CaseParamValue>();
            for (ApiRequestParam apiRequestParam : apiRequestParams) {
                CaseParamValue caseParamValue = new CaseParamValue();
//            caseParamValue.setId(apiRequestParam.getId());
                caseParamValue.setCaseId(caseEditVO.getId());
                caseParamValue.setApiRequestParamId(apiRequestParam.getId());
                caseParamValue.setApiRequestParamValue(apiRequestParam.getExampleData());
                caseParamValue.setCreateUser(user.getId());
